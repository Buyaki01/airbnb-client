    ...
    return(
      <div className="absolute inset-0 bg-white min-h-screen"> 
        <div className="p-8 grid gap-4">
          <div>
            <button onClick={() => setShowAllPhotos(false)} className=" flex gap-1 fixed bg-transparent py-2 px-4 rounded-2xl"> 
        ...
        <div className="grid gap-2 grid-cols-[2fr_1fr] rounded-2xl overflow-hidden">
          <div>
            {accomodation.photos?.[0] && (
              <div>
                <img className="aspect-square object-cover" src={`http://localhost:4000/images/${accomodation.photos?.[0]}`} alt="" />
              </div>
            )}
          </div>
          <div className="grid">
            {accomodation.photos?.[1] && (
              <img className="aspect-square object-cover" src={`http://localhost:4000/images/${accomodation.photos?.[1]}`} alt="" />
            )}

            <div className="overflow-hidden">
              {accomodation.photos?.[2] && (
                <img className="aspect-square object-cover relative top-2" src={`http://localhost:4000/images/${accomodation.photos?.[2]}`} alt="" />
              )}
            </div>
          </div>
        </div>
...
export default ShowAccomodationPage
